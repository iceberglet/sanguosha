...
import { UIPosition, Button } from "./PlayerUIAction";
...
let slashTargetFilter = (id: string, context: GameClientContext)=>{
    ...
    return id !== context.myself.player.id && 
        (context.getMyDistanceTo(id) <= (context.serverHint.slashReach || context.myself.getReach()))
}

playerActionDriverProvider.registerProvider('play-hand', (hint)=>{
    return new PlayerActionDriverDefiner('出牌阶段出杀')
            .expectChoose(UIPosition.MY_HAND, 1, 1, (id, context)=>context.myself.cardInterpreter(cardManager.getCard(id)).type.isSlash())
    ...
    return new PlayerActionDriverDefiner('出牌阶段出兵粮寸断')
            ...
            .expectChoose(UIPosition.PLAYER, 1, 1, 
                (id, context)=>id !== context.myself.player.id &&       //不能是自己
                                (context.getMyDistanceTo(id) <= (context.serverHint.ruseReach || 1)), //范围得是1
                ()=>`选择‘兵粮寸断’的对象`)
...
playerActionDriverProvider.registerProvider('play-hand', (hint)=>{
    return new PlayerActionDriverDefiner('出牌阶段出火攻')
            ...
            .expectChoose(UIPosition.PLAYER, 1, 1, 
                (id, context)=>context.getPlayer(id).getCards('hand').length > 0,   //必须对有手牌的人出
                ()=>`选择‘火攻’的对象`)
    ...
    return new PlayerActionDriverDefiner('出牌阶段出顺手牵羊')
            ...
            .expectChoose(UIPosition.PLAYER, 1, 1, 
                (id, context)=>{
                    ...
                    (context.getMyDistanceTo(id) <= (context.serverHint.ruseReach || 1))
                },
                ()=>`选择‘顺手牵羊’的对象`)
...
playerActionDriverProvider.registerProvider('play-hand', (hint)=>{
    return new PlayerActionDriverDefiner('出牌阶段穿戴装备')
            .expectChoose(UIPosition.MY_HAND, 1, 1, (id, context)=>{
                return context.myself.hp < context.myself.maxHp && cardManager.getCard(id).type.isEquipment()
            })
